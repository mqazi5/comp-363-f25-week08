...
def run_simulation():
    """
    Main simulator with 3 nested loops:
      Outer: while timer < TOTAL_TICKS
      Middle: iterate over city grid rows (i)
      Inner: iterate over intersections in each row (j)
    """

    global queue_samples, sum_queue
    global car_id, completed, sum_tt

    timer = 0  

    # run the simulation for TOTAL_TICKS time steps
    while timer < TOTAL_TICKS:
        total_waiting_tick = 0
        total_queues_tick = 0

        # loop through the city grid
        for i in range(N):  # grid rows
            for j in range(N):  # grid intersections
                node = Node(i, j)

                # move cars along outgoing links from this node
                for v, _dir_out in outgoing_for(node):
                    pop_to_queue_if_arrived(node, v)

                # serve intersection (traffic lights and turns)
                serve_intersection(timer, node)

                # generate new cars on boundary incoming links
                for v, _dir_out in outgoing_for(node):
                    if is_boundary_incoming_link(node, v):
                        if random.random() < ARRIVAL_RATE:
                            car_id += 1
                            new_car = Car(car_id, timer)
                            enqueue_departure(node, v, new_car)

                # queue statistics
                for v, _dir_out in outgoing_for(node):
                    total_waiting_tick += len(stopped[(node, v)])
                    total_queues_tick += 1

        # add this tick’s totals to global stats
        sum_queue += total_waiting_tick
        queue_samples += total_queues_tick
...
print(f"Total simulation time (ticks): {TOTAL_TICKS}")
print(f"Signal cycle: NS={CYCLE_NS_GREEN}, EW={CYCLE_EW_GREEN}, total={CYCLE_TOTAL}")
print(f"Boundary arrival rate per link: {ARRIVAL_RATE}")
print(f"Total trips completed: {completed}")

throughput = completed / TOTAL_TICKS if TOTAL_TICKS else 0.0
print(f"Throughput (completed/TOTAL_TICKS): {throughput}")

mean_tt = (sum_tt / completed) if completed else 0.0
print(f"Mean travel time (sum_tt/completed): {mean_tt}")

mean_queue = (sum_queue / max(1, queue_samples))
print(f"Average queue size (sum_queue/queue_samples): {mean_queue}")
